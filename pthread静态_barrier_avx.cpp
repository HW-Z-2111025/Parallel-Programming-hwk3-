    ...
    for (int k = 0; k < N; k++) {
        if (t_id == 0) {
            ...
            for (int j = k + 1; j < N; j += 8)
            {
                ...
                va = _mm256_loadu_ps(&mtx[k][j]);
                va = _mm256_div_ps(va, vt);
                _mm256_storeu_ps(&mtx[k][j], va);
            }
            mtx[k][k] = 1.0;
        }
        ...
        for (int i = k + 1 + t_id; i < N; i += numThread) {
            ...
            for (int j = k + 1; j < N; j += 8)
            {
                ...
                vakj = _mm256_loadu_ps(&mtx[k][j]);
                vaij = _mm256_loadu_ps(&mtx[i][j]);
                vx = _mm256_mul_ps(vakj, vaik);
                vaij = _mm256_sub_ps(vaij, vx);
                _mm256_storeu_ps(&mtx[i][j], vaij);
            }
            mtx[i][k] = 0;
        }
